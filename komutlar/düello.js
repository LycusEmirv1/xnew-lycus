...
exports.run = async (client, message, args) => {
  ...
  if (talkedRecently.has(message.author.id)) {
           return message.channel.send("5 Dakikada Bir Kullanabilirsin - " + message.author);
    } else {

       if (!ayarlar.sahip.includes(message.author.id) && message.author.id != "254579165003644929") { talkedRecently.add(message.author.id);
        setTimeout(() => {
        message.delete();
