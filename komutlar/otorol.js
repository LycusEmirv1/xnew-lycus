...
exports.run = async (client, message, args) => {
    ...
    if(args[0] === 'rol-ayarla') {
       ...
       if(!rol) return message.channel.send('**Bir rol ismi veya id si girmediniz**');
        ...
        message.channel.send(`**Oto rol başarılı bir şekilde ${rol} olarak ayarlandı**`)
      
    } else if(args[0] == 'rol-sıfırla') {
        if(!db.has(`${message.guild.id}_otorol`)) return message.channel.send('**Zaten oto rol ayarlanmamış**'); else {
            db.delete(`${message.guild.id}_otorol`)
            message.channel.send('** ${client.emojis.get("731902694230327418")} Oto rol başarılı bir şekilde sıfırlandı**')
        }
        ...
        if(!kanal) return message.channel.send('Bir kanal etiketlemediniz'); else {
            ...
            message.channel.send(`**${client.emojis.get("731902694230327418")} Oto rol kanal başarılı bir şekilde ${kanal} olarak ayarlandı**`)
        }
    } else if(args[0] === 'kanal-sıfırla') {
        if(!db.has(`${message.guild.id}_otokanal`)) return message.channel.send('**Zaten oto rol kanal ayarlanmamış**'); else {
            db.delete(`${message.guild.id}_otokanal`)
            message.channel.send('** ${client.emojis.get("731902694230327418")} Oto rol kanal başarılı bir şekilde sıfırlandı**')
        }
    }
}
exports.conf = {
